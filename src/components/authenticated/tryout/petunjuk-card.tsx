...
const PetunjukCard = () => {
  ...
  return (
    <div className='bg-primary-background flex flex-col box-border w-full w-max-[398px] h-auto md:w-[840px] rounded-[20px] p-6 gap-[44px]'>
      ...
      <div>
        {isLocked ? (
          <Button variant='disabled' disabled className='w-full bg-neutral-400 cursor-not-allowed text-neutral-600 font-bold'>
            Belum bisa memulai tryout
          </Button>
        ) : (
          <Link href="/tryout">
            <Button variant='primary' className='w-full bg-primary-600 cursor-pointer font-bold'>Mulai</Button>
          </Link>
        )}
      </div>
    </div>
